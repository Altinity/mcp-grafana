...
- ClickHouse tools (`query_clickhouse`, `list_clickhouse_tables`, `describe_clickhouse_table`) now also work with `vertamedia-clickhouse-datasource` ([Altinity/clickhouse-grafana](https://github.com/Altinity/clickhouse-grafana)). Discovery still goes through Grafana (`getDatasourceByUID` — datasource must exist and be visible to the calling user); the query itself is issued directly to ClickHouse at the URL configured on the datasource, with the inbound user bearer forwarded as `Authorization: Bearer <jwt>`. ClickHouse validates the JWT via its own JWKS / `token_processor`. This bypasses Grafana's datasource proxy because Grafana's `oauthPassThru` does not compose with `auth.jwt` — it only forwards a token when the user has a Grafana-managed OAuth session. Fork-only; not a candidate for upstream PR.
